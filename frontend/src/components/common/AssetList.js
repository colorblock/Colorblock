      ...
      <ul data-role='blocks' className={`${flow} justify-between text-xs`}>
        {
          assets.map((asset) => (
            <li 
              ...
              onClick={ () => document.location.href = `/asset/${asset.id}` }
              key={asset.id}
            >
              ...
              <div data-role='thumbnail' className='h-32 py-6 flex justify-center'>
                <img className='h-full' src={asset.item && `${serverUrl}/static/img/${asset.item.id}.${asset.item.type === 0 ? 'png' : 'gif'}`} alt={asset.title} />
              </div>
              <div data-role='owner and price' className='h-8 flex assets-end justify-between'>
                <div>
                  <p className='text-xxs text-gray-500'>Owner</p>
                  <p>{asset.user_id ? `${asset.user_id.slice(0, 4)}....${asset.user_id.slice(-4)}` : 'NULL'}</p>
                </div>
